...
#
# opto-sync-api-server — production container image.
#
...
#   docker buildx build --platform linux/arm64 -t ghcr.io/opto-sync/opto-sync-api-server:dev --load .
#
...
#     -t ghcr.io/opto-sync/opto-sync-api-server:dev --load .
#
#   # OCI image index with provenance and SBOM attestations:
#   docker buildx build --platform linux/arm64 --push \
#     --provenance=true --sbom=true \
#     --output type=image,oci-mediatypes=true,name=ghcr.io/opto-sync/opto-sync-api-server:dev .
#
...
#     ghcr.io/opto-sync/opto-sync-api-server:dev
#
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry,sharing=locked \
    ...
    cargo build --release --bin opto-sync-api-server; \
    strip target/release/opto-sync-api-server; \
    cp target/release/opto-sync-api-server /usr/local/bin/opto-sync-api-server
...
COPY --from=builder --chown=65532:65532 /usr/local/bin/opto-sync-api-server /usr/local/bin/opto-sync-api-server
ENV OPTO_SYNC_API_BIND=0.0.0.0:8080
EXPOSE 8080
USER 65532:65532
ENTRYPOINT ["/usr/local/bin/opto-sync-api-server"]
...
COPY --from=builder /usr/local/bin/opto-sync-api-server /usr/local/bin/opto-sync-api-server
...
USER 10001:10001
ENTRYPOINT ["/usr/local/bin/sops-entrypoint.sh", "/usr/local/bin/opto-sync-api-server"]
CMD []
