...
#
# Multi-stage image for opto-sync-api-server.
# Prefer linux/arm64:
#   docker buildx build --platform linux/arm64 -t opto-sync-api-server:dev .
#   docker run --rm --platform linux/arm64 \
#     -e SOPS_AGE_KEY="$(cat ~/.config/sops/age/keys.txt)" opto-sync-api-server:dev
#
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry,sharing=locked \
    ...
    --mount=type=cache,target=/src/target,id=opto-sync-api-server-target-${TARGETARCH},sharing=locked \
    cargo build --locked --release --bin opto-sync-api-server \
...
COPY --from=build "/usr/local/bin/opto-sync-api-server" "/usr/local/bin/opto-sync-api-server"
...
RUN --mount=type=bind,source=.,target=/src,ro \
    ...
    && chown -R 65532:65532 /app /usr/local/bin/opto-sync-api-server
ENV SOPS_SECRETS_FILE=/app/secrets/app.env \
    OTEL_SERVICE_NAME=opto-sync-api-server \
