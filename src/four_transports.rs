#![forbid(unsafe_code)]
//! Serving the four avenues from `opto-sync-api-server`.
//!
...
//! |---|------|---------------------|
//! | 1 | `direct_read` | none: the web server reads Postgres itself through `opto-sync-lib-core` |
...
pub const SERVICE_SLUG: &str = "opto-sync";

/// Environment prefix for every variable this service reads.
pub const ENV_PREFIX: &str = "OPTO_SYNC";
...
///
/// Bind `OPTO_SYNC_API_MTLS_ADDR` and pass the listener. One task per
...
/// Open the JetStream context for avenue 4 from `OPTO_SYNC_NATS_URL`.
///
/// # Errors
