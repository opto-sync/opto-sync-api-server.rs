...
impl DomainConfig {
    ...
    fn parse(source: &str) -> Result<Self, String> {
        ...
        for raw_line in source.lines() {
            ...
            let (key, value) = line
                .split_once('=')
                .ok_or_else(|| format!("{DOMAIN_CONFIG_NAME} is invalid"))?;
            let key = key.trim();
        ...
        if contract != ".cli-flags.toml"
            || require_audit != Some(true)
            || precedence.as_deref() != Some("argv-over-env")
        {
            return Err(format!(
                "{DOMAIN_CONFIG_NAME} must select audited .cli-flags.toml with argv-over-env precedence"
            ));
        }
    ...
    fn apply(&self, env: &mut EnvMap) -> Result<(), String> {
        for binding in &self.bindings {
            ...
            if let Some(default) = &binding.default {
                ...
                return Err(format!(
                    "{DOMAIN_CONFIG_NAME} required binding {} is missing",
                    binding.name
                ));
            }
        }
...
impl BindingBuilder {
    ...
    fn finish(self) -> Result<DomainBinding, String> {
        let name = self
            .name
            .ok_or_else(|| format!("{DOMAIN_CONFIG_NAME} env binding has no name"))?;
        let binding = DomainBinding {
            key: self
                .key
                .ok_or_else(|| format!("{DOMAIN_CONFIG_NAME} binding {name} has no key"))?,
            kind: self
                .kind
                .ok_or_else(|| format!("{DOMAIN_CONFIG_NAME} binding {name} has no kind"))?,
            required: self
                .required
                .ok_or_else(|| format!("{DOMAIN_CONFIG_NAME} binding {name} has no required flag"))?,
            secret: self
                .secret
                .ok_or_else(|| format!("{DOMAIN_CONFIG_NAME} binding {name} has no secret flag"))?,
            default: self.default,
            name,
        };
        if binding.secret && binding.default.is_some() {
            return Err(format!(
                "{DOMAIN_CONFIG_NAME} secret binding {} may not have a plaintext default",
                binding.name
            ));
        }
        Ok(binding)
...
fn finish_binding(
    current: &mut Option<BindingBuilder>,
    bindings: &mut Vec<DomainBinding>,
) -> Result<(), String> {
    if let Some(builder) = current.take() {
        bindings.push(builder.finish()?);
    }
    Ok(())
}
...
fn validate_bindings(bindings: &[DomainBinding]) -> Result<(), String> {
    ...
    for binding in bindings {
        if !valid_name(&binding.name)
            || !valid_env_key(&binding.key)
            || !matches!(
                binding.kind.as_str(),
                "string" | "bool" | "integer" | "double" | "json" | "url"
            )
        {
            return Err(format!(
                "{DOMAIN_CONFIG_NAME} binding {} is invalid",
                binding.name
            ));
        }
        if !names.insert(binding.name.as_str()) || !keys.insert(binding.key.as_str()) {
            return Err(format!(
                "{DOMAIN_CONFIG_NAME} binding {} is duplicated or aliases another binding",
                binding.name
            ));
        }
    }
...
fn set_once<T>(slot: &mut Option<T>, value: T) -> Result<(), String> {
    if slot.is_some() {
        return Err(format!("{DOMAIN_CONFIG_NAME} contains a duplicate field"));
    }
    *slot = Some(value);
    Ok(())
}
...
fn parse_u32(value: &str) -> Result<u32, String> {
    value
        .parse::<u32>()
        .map_err(|_| format!("{DOMAIN_CONFIG_NAME} contains an invalid integer"))
}
...
fn parse_bool(value: &str) -> Result<bool, String> {
    match value {
        "true" => Ok(true),
        "false" => Ok(false),
        _ => Err(format!("{DOMAIN_CONFIG_NAME} contains an invalid boolean")),
    }
}
...
fn parse_string(value: &str) -> Result<String, String> {
    let value = value
        ...
        .ok_or_else(|| format!("{DOMAIN_CONFIG_NAME} common envelope requires quoted strings"))?;
    if value.is_empty()
        || value
            .chars()
            .any(|character| character.is_control() || character == '\\')
    {
        return Err(format!("{DOMAIN_CONFIG_NAME} contains an unsafe string"));
    }
    Ok(value.to_owned())
}
...
fn strip_comment(line: &str) -> Result<&str, String> {
    ...
    for (index, byte) in line.bytes().enumerate() {
        if byte == b'"' {
            ...
            return Err(format!(
                "{DOMAIN_CONFIG_NAME} common envelope does not permit escaped strings"
            ));
        }
    }
    if quoted {
        Err(format!("{DOMAIN_CONFIG_NAME} has an unterminated string"))
    } else {
        Ok(line)
    }
}
...
pub fn apply_cli_flags_from(
    ...
    if config_path.file_name().and_then(|value| value.to_str()) != Some(domain.contract.as_str()) {
        return Err(format!(
            "{DOMAIN_CONFIG_NAME} flags2env contract does not match the executable contract path"
        ));
    }
...
mod tests {
    ...
    fn domain_manifest_controls_runtime_default() {
        let env = apply_cli_flags_from(vec!["svc".into()], EnvMap::new(), &config_path())
            .expect("embedded Opto Sync config must be executable");
        assert_eq!(value(&env, "OPTO_SYNC_API_BIND"), Some("127.0.0.1:8080"));
    }
    ...
    fn domain_manifest_rejects_wrong_mode_and_secret_defaults() {
        ...
        let secret_default = DOMAIN_CONFIG_SOURCE.replacen(
